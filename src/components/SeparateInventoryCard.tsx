...
import { Package, Snowflake, Coffee, AlertCircle } from 'lucide-react'
...
export default function SeparateInventoryCard({ inventoryStatus, onForceRefresh }: SeparateInventoryCardProps) {
  ...
  if (combinedData.totalProduced > 0 && refrescosData.totalProduced === 0 && heladosData.totalProduced === 0) {
    return (
    ...
    <Card className="w-full">
      <CardHeader>
        ...
        <p className="text-gray-600">
          Refrescos vs Helados - Inventario separado con costos individuales
        </p>
      </CardHeader>
        ...
        <div className="border-2 border-orange-200 rounded-lg p-4 bg-orange-50">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-xl font-bold text-orange-800">🍦 HELADOS</h3>
            </div>
            <Badge className="bg-orange-600 text-white">$1,800 c/u</Badge>
          </div>
        ...
        <div className="border-2 border-gray-300 rounded-lg p-4 bg-gray-50">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center p-3 bg-white rounded">
              ...
              <p className="text-xs text-gray-500">{refrescosData.totalProduced} + {heladosData.totalProduced}</p>
            </div>
            <div className="text-center p-3 bg-white rounded">
              <p className="text-sm text-gray-600">TOTAL VENDIDO</p>
              <p className="text-2xl font-bold text-red-600">{combinedData.totalSold}</p>
              <p className="text-xs text-gray-500">{refrescosData.totalSold} + {heladosData.totalSold}</p>
            </div>
            <div className="text-center p-3 bg-white rounded">
              <p className="text-sm text-gray-600">TOTAL STOCK</p>
              <p className="text-2xl font-bold text-green-600">{combinedData.currentInventory}</p>
              <p className="text-xs text-gray-500">{refrescosData.currentInventory} + {heladosData.currentInventory}</p>
            </div>
            <div className="text-center p-3 bg-white rounded">
